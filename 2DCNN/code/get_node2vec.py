...
def get_embeddings_node2vec(g,d,p,q,path_node2vec):
    ...
    # execute node2vec
    #call([path_node2vec + 'node2vec -i:' + tmpdir + '/graph/input.edgelist' + ' -o:' + tmpdir + '/emb/output.emb' + ' -p:' + p + ' -q:' + q + ' -d:10'],shell=True)
...
def to_parallelize(file_name,p,q,dataset,path_read,path_write):
    ...
    #in case the adjm txt is empty
    # if adj_mat.size < 25:
    #     with open('/Volumes/ExFAT256/error.txt','ab') as f:
    #         np.savetxt(f,idx)
    #     f.close()
    #     return None

    g = igraph.Graph.Adjacency(adj_mat.tolist(),mode='UNDIRECTED')
    #print (file_name,adj_mat.shape,len(g.vs),max_n_channels*2)
...
def main():
    ...
    try:
        ...
        pass
    try:
        file_names.remove('._Store')
    except:
        pass
        
    exsit_files = os.listdir(path_write + dataset+ '/')
    print('总共%d，已有%d'%(len(file_names),len(exsit_files)))
    name_len = dataset + '_node2vec_raw_p=' + p + '_q=' + q + '_'
    for f in file_names[:]:
        fcheck = name_len+f[:-4] + '.npy'
        if fcheck in exsit_files:
            file_names.remove(f)
    ...
    #     print('+++++++++++++++++++',fil)
    #     to_parallelize(fil,p,q,dataset,path_read,path_write)
