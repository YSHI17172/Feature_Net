...
def main():
    ...
    for idx,my_file_name in enumerate(file_names_filtered):
        emb = tensors[idx]
        emb = emb[:,:n_dim]
        my_hist = get_hist_node2vec(emb=emb,d=n_dim,my_min=my_min,my_max=my_max,definition=definition) 
        tensors[idx] = my_hist
    ...
    np.savez_compressed(path_root + '/results/backup/' + dataset+ my_date_time + '_folds',
    folds = folds,labels=folds_labels,shuffled_idxs=shuffled_idxs,input_shape=input_shape)    
