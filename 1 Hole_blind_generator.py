...
for mesh_number in range(1,101):
    ...
    print ("Blind Hole Mesh %d, depth %.2f, radius %.2f, has %d points."%(mesh_number,d2,r2,coord_array.shape[0]))
    np.savez_compressed('input_mesh/b_hole/b_hole_%05d-1'%mesh_number, coord_array=coord_array,tri_array=tri_array.astype(np.int32))
    ...
    for i in range(9): #随机旋转模型
        ...
        np.savez_compressed('input_mesh/b_hole/b_hole_%05d-%d'%(mesh_number,i+2), coord_array=new_coord_array,tri_array=tri_array.astype(np.int32))
