...
for mesh_number in range(1,101):
    ...
    d2 = np.random.randint(10*100,90*100)/100 # 深度
    ...
    print ("Protrusion Mesh %d, depth %.2f, width %.2f, length %.2f, has %d points."%(mesh_number,d2,w2,l2,coord_array.shape[0]))
    np.savez_compressed('input_mesh/protrusion/protrusion_%05d-1'%mesh_number,
    ...
    for i in range(9): #随机旋转模型
        ...
        np.savez_compressed('input_mesh/protrusion/protrusion_%05d-%d'%(mesh_number,i+2),
